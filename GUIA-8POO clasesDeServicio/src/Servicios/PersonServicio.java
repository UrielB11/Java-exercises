...
while (!(sex == 'H' || sex == 'M' || sex == 'O')) {
    System.out.println("El dato ingresado no es valido. Ingrese nuevamente:");
    sex = sc.next().toUpperCase().charAt(0);
        ...
        return new Person(nom, edad, sex, peso, alt);
    }
    //Método esMayorDeEdad(): indica si la persona es mayor de edad. La función devuelve un booleano.
    public boolean esMayorDeEdad(Person p){
    return p.getEdad()>=18;
    }
    /*Método calcularIMC(): calculara si la persona está en su peso ideal (peso en kg/(altura^2 en mt2)). 
Si esta fórmula da por resultado un valor menor que 20, significa que la persona está por debajo 
...
//            System.out.println("La persona se encuentra en el peso ideal");
            return 0;
        }
        else {
//            System.out.println("La persona se encuentra por encima del peso ideal");
            return 1;
        } 
    }
}